...
export default function BookDetails() {
  ...
  useEffect(() => {
    ...
    fetchBook(lang, slug).then(setBook).catch(() => setError(true));
  }, [lang, slug]);
  ...
  const displayLanguage = labels.languageNames?.[book?.language] || book?.language || "—";
  const displayCategory = labels.categoryNames?.[book?.category] || book?.category || "—";
    ...
    >
      ...
      <nav className="bg-[#fffaf0] border-b border-[#e8d99b] px-6 py-4">
        <ul className="list-none m-0 p-0 flex flex-wrap justify-center gap-8">
          <li>
            <Link className="text-[#5c4712] font-semibold no-underline hover:text-[#c3a421] transition-colors" to="/">
              {labels.home}
            </Link>
          </li>
          <li>
            <Link className="text-[#5c4712] font-semibold no-underline hover:text-[#c3a421] transition-colors" to={labels.libraryUrl}>
              {labels.library}
            </Link>
          </li>
        </ul>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-7">
              {sections.map((section) => {
                ...
                return (
                  ...
                  >
                    <span className="block text-[1rem] md:text-[1.05rem]">{section.title}</span>
                  </button>
                );
              })}
            </section>
...
function SectionHeading({ title }) {
  return (
    <div className="mb-6">
      <h3 className="text-[1.35rem] md:text-[1.5rem] font-bold text-[#5c4712]">{title}</h3>
    </div>
  );
}